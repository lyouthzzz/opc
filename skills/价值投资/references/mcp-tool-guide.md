# 金融数据 MCP 工具使用手册（Tushare 兼容 MCP）

本手册记录「市场现状分析」环节所使用的 **Tushare 兼容 MCP**（`tushareMcp`）工具的参数与查询模式。价值投资分析在完成生意本质、五维度定性分析后，用这些工具补充**实时市场数据**（最近股价、K 线、年报/财报三大表、财务指标、估值、分红、股东、公告/研报、资金流、宏观与国债收益率），使 right price 判断落到可验证的数据上。

Tushare 兼容 MCP 暴露的工具对应 Tushare 数据接口，覆盖 **A 股 / 港股 / 美股**的行情与财务，以及公告、研报、宏观等。本手册只收录价值投资常用子集。
...
价值投资分析依赖**当前**市场与财报事实。以下流程对应 SKILL 的「第零步」，必须在调用行情/财报接口之前完成。
...
价值投资分析在定性框架之外，用以下固定顺序补充市场现状（对应 SKILL 的「市场现状分析」步骤）。**先完成第二节锚点查询**，再按序拉数；**日期用 `YYYYMMDD`**。
