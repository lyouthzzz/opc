...
2. 用普通用户友好的方式询问是否配置数据增强（必须展示选择，但允许分别跳过）：
   ...
   - 遇到文件冲突时，用一句话说明“会更新什么、为什么”，通过系统确认框或简短问题征得同意；不要直接展示 diff，除非用户要求查看技术细节。
   ...
   - CLI 就绪后，把 `announcement-search` 安装到每个目标 skills 目录；已存在时先比较，用普通语言征得同意后再升级或覆盖。
...
3. 安装本仓库 skills：Windows 优先由你在后台调用 `scripts/install-skills.ps1`，macOS / Linux 优先调用 `scripts/install-skills.sh`；按目标 Agent 复制每个含 SKILL.md 的完整目录（包括 references/），保持目录名与结构不变。
...
Windows 脚本对应参数为 `-Target`、`-Force`、`-DryRun`、`-NoCommands`、`-CommandsOnly`、`-Destination`。Windows 版本固定使用复制安装；冲突时默认用系统窗口确认，更新前备份原内容。
